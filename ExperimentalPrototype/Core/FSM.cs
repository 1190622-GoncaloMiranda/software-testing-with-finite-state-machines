...
/// transitions are treated as undefined behaviour at execution time. Non-deterministic
/// or total-function variants are out of scope.
/// </summary>
public class FSM
{
    ...
    // Index for fast lookup: from state name -> outgoing transitions
    ...
    /// <summary>Returns all outgoing transitions from the given state.</summary>
    ...
    /// <summary>Returns all outgoing transitions from the given state name.</summary>
    ...
    /// <summary>Tries to find the transition from a given state on a given input.</summary>
    ...
    /// <summary>Returns all states reachable from the initial state via BFS.</summary>
    ...
    /// <summary>Deep clones this FSM, creating fresh State and Transition objects.
    /// Used by mutation operators to avoid modifying the original.</summary>
    public FSM Clone()
    {
    ...
    /// <summary>Creates a new FSM with the same states and initial state, but a
    /// replaced transition list. Ensures the internal adjacency index is rebuilt
    /// so that GetTransition reflects the modified transitions. Used by mutation
    /// operators to produce mutants whose behavioural changes are actually visible
    /// at execution time.</summary>
