...
namespace ExperimentalPrototype.Strategies.Prioritization;
...
public class CoverageBasedPrioritization : IPrioritizationStrategy
{
    ...
    public TestSuite Prioritize(TestSuite suite, FSM fsm)
    {
        ...
        {
            // Pick the test case that covers the most NEW transitions
            var best = remaining
...
public class WeightedTransitionPrioritization : IPrioritizationStrategy
{
    ...
    public TestSuite Prioritize(TestSuite suite, FSM fsm)
    {
        // Compute per-transition weights once: weight = 1 / (number of test cases covering it)
        ...
        while (remaining.Count > 0)
        {
            // Pick the test case with the highest total weight of newly covered transitions
            var best = remaining
                .Select(tc => (tc, score: tc.CoveredTransitions
