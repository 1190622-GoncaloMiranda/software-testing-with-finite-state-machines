...
namespace ExperimentalPrototype.Tests.Mutation;
...
public class MutationEngineTests
{
    ...
    public void GenerateMutants_AggregatesMutantsFromAllOperators()
    {
        ...
        op1.Apply(Arg.Any<ExperimentalPrototype.Core.FSM>()).Returns(new List<Mutant> { m1, m2 });

        var op2 = Substitute.For<IMutationOperator>();
        op2.Name.Returns("Op2");
        op2.Apply(Arg.Any<ExperimentalPrototype.Core.FSM>()).Returns(new List<Mutant> { m3 });
    ...
    public void GenerateMutants_CallsApplyOnEachOperatorExactlyOnce()
    {
        ...
        var op1 = Substitute.For<IMutationOperator>();
        op1.Name.Returns("Op1");
        op1.Apply(Arg.Any<ExperimentalPrototype.Core.FSM>()).Returns(new List<Mutant>());

        var op2 = Substitute.For<IMutationOperator>();
        op2.Name.Returns("Op2");
        op2.Apply(Arg.Any<ExperimentalPrototype.Core.FSM>()).Returns(new List<Mutant>());

        new MutationEngine(new[] { op1, op2 }).GenerateMutants(fsm);

        op1.Received(1).Apply(Arg.Any<ExperimentalPrototype.Core.FSM>());
        op2.Received(1).Apply(Arg.Any<ExperimentalPrototype.Core.FSM>());
    }
    ...
    public void GenerateMutants_OperatorReturnsEmpty_ContributesNoMutants()
    {
        ...
        op.Apply(Arg.Any<ExperimentalPrototype.Core.FSM>()).Returns(new List<Mutant>());
    ...
    {
        var fsm = FsmFixtures.Toggle2(); // 2 transitions
    ...
    {
        var fsm = FsmFixtures.Toggle2(); // 2 states → 1 alternative initial state
